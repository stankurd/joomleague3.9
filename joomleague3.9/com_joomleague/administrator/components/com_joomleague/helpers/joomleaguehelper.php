...
// return cached or uncached (if not necessary) images
	...
	/*
	 * public static function getTimestamp($date = null, $offset=null)
	{
	    $date = $date ? $date : 'now';
	    $app = Factory::getApplication();
...
static $_foldertype = '';
  ...
  self::$_foldertype = $type;
	...
	public static function getSelector( $fieldname, $fieldpreview_name, $type, $value, $default = '', $control_name='', $fieldid)
	{
		...
		$baseFolder = Uri::root();//.'media/com_joomleagueE/database/'.ImageSelect::getfolder($type);
		...
		function selectImage_" . $type . "(image, imagename, field, fieldid)
		{
			...
			document.getElementById('a_' + field).value = 'images/com_joomleague/database/" . self::getfolder( $type ) . "/'+image;
			document.getElementById('a_' + field + '_name').value ='images/com_joomleague/database/" . self::getfolder( $type ) . "/'+imagename;
			document.getElementById('" . $fieldpreview_name . "').src = '".Uri::root()."images/com_joomleague/database/" . self::getfolder( $type ) . "/'+image;
var els=document.getElementsByName(field);
for (var i=0;i<els.length;i++) {
els[i].value = 'images/com_joomleague/database/" . self::getfolder( $type ) . "/'+imagename;}

			//$('a_' + field).value = 'images/com_joomleague/database/" . self::getfolder( $type ) . "/'+image;
//			$('a_' + field + '_name').value ='images/com_joomleague/database/" . self::getfolder( $type ) . "/'+imagename;
			//$('a_' + field + '_name').fireEvent('change');
      		//if($(fieldid)) {
        		//$(fieldid).value = 'images/com_joomleague/database/" . self::getfolder( $type ) . "/'+imagename;
//      		}
	//		$('a_' + field + '_name').fireEvent('change');
		}
		...
		function clear_" . $funcname . "()
		{
			...
			document.getElementById('" . $fieldpreview_name . "').src = '/media/COM_JOOMLEAGUE/jl_images/spinner.gif';
...
//			jQuery('#a_" . $fieldname . "_name').change();
		...
		$type . '&amp;field=' . $fieldname . '&amp;fieldid=' . $fieldid .'&amp;tmpl=component';
/*		
$link2 = 'index.php?option=com_media&tmpl=component&path=local-0:/com_joomleague/database/'.$type;
$link2 = 'index.php?option=com_media&view=images&tmpl=component&asset=com_joomleague&author=&folder=com_joomleague/database/'.$type;
...
*/		
        ...
        $imageselect .=	 JoomleagueHelper::getBootstrapModalImage('upload'.$funcname ,Text::_('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_TITLE'),Text::_('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_TITLE'),'20',Uri::base().$link,$modalwidth,$modalheight);   		
		$imageselect .=	 "</div></div>\n";
		/*
		...
		*/
        ...
        $imageselect .=	 JoomleagueHelper::getBootstrapModalImage('select'.$funcname ,Text::_('JLIB_FORM_MEDIA_PREVIEW_SELECTED_IMAGE'),Text::_('JLIB_FORM_MEDIA_PREVIEW_SELECTED_IMAGE'),'20',Uri::base().$link2,$modalwidth,$modalheight);   		
		$imageselect .=	 "</div></div>\n";
		/*
...
/*
		...
		{
			$app->enqueueMessage( 100, Text::_( 'COM_JOOMLEAGUE_ADMIN_IMAGEHANDLER_UPLOAD_FAILED' ) . ': ' . htmlspecialchars($file['name'], ENT_COMPAT, 'UTF-8' ) );
			return false;
		}
*/
		...
		{
			$app->enqueueMessage( 100, Text::_( 'COM_JOOMLEAGUE_ADMIN_IMAGEHANDLER_ERROR1' ) . ': ' . htmlspecialchars( $file['name'], ENT_COMPAT, 'UTF-8' ) );
			return false;
		}
		...
		{
			$app->enqueueMessage( 100, Text::_( 'COM_JOOMLEAGUE_ADMIN_IMAGEHANDLER_ERROR2' ) . ': ' . htmlspecialchars( $file['name'], ENT_COMPAT, 'UTF-8' ) );
			return false;
		}
		...
		foreach( $html_tags as $tag )
		{
			...
			{
				$app->enqueueMessage( 100, Text::_( 'COM_JOOMLEAGUE_ADMIN_IMAGEHANDLER_IE_WARN' ) );
				return false;
			}
		}
