...
//     echo 'configavatar <br /><pre>~' . print_r($configavatar,true) . '~</pre><br />';
		...
		if (!$result=$this->_db->loadResult())
		{
			if (isset($this->predictionGame) && ($this->predictionGame->master_template))
			{
              ...
              ON p.id=t.prediction_id
				...
				if (!$result=$this->_db->loadResult())
				{
					JError::raiseNotice(500,JText::sprintf('COM_JOOMLEAGUE_PRED_MISSING_MASTER_TEMPLATE',$template,$predictionGame->master_template));
					JError::raiseNotice(500,JText::_('COM_JOOMLEAGUE_PRED_MISSING_MASTER_TEMPLATE_HINT'));
					echo '<br /><br />';
					return false;
				}
			}
			...
			{
				JError::raiseNotice(500,JText::sprintf('COM_JOOMLEAGUE_PRED_MISSING_TEMPLATE',$template,$this->predictionGameID));
				JError::raiseNotice(500,JText::_('COM_JOOMLEAGUE_PRED_MISSING_MASTER_TEMPLATE_HINT'));
				echo '<br /><br />';
				return false;
			}
	...
	function getTimestamp($date,$offset=0)
	{
		...
		{
			$datum=preg_split("/-| |:/",JHTML::_('date',date('Y-m-d H:i:s',time()),"Y-m-d H:i:s"));
		}
		if ($offset)
		{
...
//  $app->enqueueMessage(JText::_('roundResults -> <pre> '.print_r($roundResults,true).'</pre><br>' ),'Notice');
//  $app->enqueueMessage(JText::_('predictionProject -> <pre> '.print_r($predictionProject,true).'</pre><br>' ),'Notice');
//  $app->enqueueMessage(JText::_('predictionProjectS -> <pre> '.print_r($predictionProjectS,true).'</pre><br>' ),'Notice');
                                        
  $predictionGameMemberMail = $this->getPredictionMemberEMailAdress($predictionMemberID);
  //$app->enqueueMessage(JText::_('predictionGameMemberMail -> <pre> '.print_r($predictionGameMemberMail,true).'</pre><br>' ),'Notice');
...
$body .= $this->createHelptText($predictionProject->mode);
  ...
  //Sending the mail
	...
	$app->enqueueMessage(JText::_('COM_JOOMLEAGUE_PRED_ENTRY_MAIL_SEND_ERROR'),'Error');
	}
	else
	{
	//echo 'Mail sent';
	$emailadresses = implode(",",$predictionGameMemberMail);
	$app->enqueueMessage(JText::sprintf('COM_JOOMLEAGUE_PRED_ENTRY_MAIL_SEND_OK',$emailadresses),'');
	}
...
*/
    ...
    //$output='<select class="inputbox" id="p" name="p" onchange="document.forms[\'resultsRoundSelector\'].r.value=0;submit();" >';
		...
		if (isset($addTotalSelect))
		{
			...
			$output .= '>'.JText::_('COM_JOOMLEAGUE_PRED_TOTAL_RANKING').'</option>';
		}
		else
		{
			$addTotalSelect=1;
